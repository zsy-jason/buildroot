...
def main():
  ...
  # Sorted the sdk_json by sdkPath to stabilize the output order of xcodebuild,
  # preventing unexpected changes in the SDK path and avoiding a full rebuild
  # when GN files are modified.
  sdk_json = sorted(sdk_json, key=lambda x: x['sdkPath'])
  ...
  if options.symlink or options.print_sdk_path:
    if options.symlink:
      symlink_target = os.path.join(
          options.symlink, 'SDKs', os.path.basename(sdk_output))
