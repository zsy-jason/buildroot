...
def _ParseArgs(args):
  ...
  input_opts.add_argument(
      '--generate-on-resources-loaded',
      action='store_true',
      help='Generate an onResourcesLoaded() method in the R.java source file even at a final apk.')
...
def _FixManifest(options, temp_dir):
  ...
  with open(debug_manifest_path, 'w') as debug_manifest:
    debug_manifest.write(ElementTree.tostring(
        original_manifest.getroot(), encoding='UTF-8').decode())
...
def _CreateResourceInfoFile(
  ...
  for zip_file in dependencies_res_zips:
    ...
    if os.path.exists(zip_info_file_path):
      with open(zip_info_file_path, 'rb') as zip_info_file:
        lines.update(zip_info_file.readlines())
  for dest, source in renamed_paths.items():
    lines.add('Rename:{},{}\n'.format(dest, source).encode())
...
def main(args):
  ...
  with resource_utils.BuildContext(debug_temp_resources_dir) as build:
    ...
    if options.shared_resources_whitelist:
      ...
      rjava_build_options.GenerateOnResourcesLoaded()
    elif options.generate_on_resources_loaded:
      rjava_build_options.GenerateOnResourcesLoaded()
