...
def main(argv):
  ...
  if is_static_library_dex_provider_target:
    ...
    for config_path, dep_config in (
        iter(static_library_dependent_configs_by_path.items())):
  ...
  if is_apk_or_module_target:
    ...
    config['extra_android_manifests'] = [_f for _f in (
        d.get('android_manifest') for d in all_resources_deps) if _f]
