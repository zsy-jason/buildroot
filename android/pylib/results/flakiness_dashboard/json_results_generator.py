...
class JSONResultsGeneratorBase(object):
  ...
  def _GetArchivedJSONResults(self):
    ...
    results_file_url = (self.URL_FOR_TEST_LIST_JSON %
                        (urllib.parse.quote(self._test_results_server),
                         urllib.parse.quote(self._builder_name),
                         self.RESULTS_FILENAME,
                         urllib.parse.quote(self._test_type),
                         urllib.parse.quote(self._master_name)))

    try:
      # FIXME: We should talk to the network via a Host object.
      results_file = urllib.request.urlopen(results_file_url)
