...
def _CheckLoadAddressRandomization(lib_map_list, process_type):
  ...
  for lib_map in lib_map_list:
    for lib_name, lib_address in lib_map.items():
  ...
  bad_libs = {}
  for lib_name, lib_address_list in lib_addr_map.items():
  ...
  def _RunTest(self, device):
    ...
    if browser_set != renderer_set:
      logging.error('Library set mistmach browser=%s renderer=%s',
          list(browser_libs.keys()), list(renderer_libs.keys()))
    ...
    bad_libs = []
    for lib_name, lib_address in renderer_libs.items():
    ...
    if browser_config == 'ALWAYS' or \
      ...
      addr_mismatches = []
      for lib_name, lib_address in browser_libs.items():
