...
class TestSurfaceStatsCollector(unittest.TestCase):
  ...
  def testOneFrameDelta(self):
    ...
    self.assertEqual(results['avg_surface_fps'].value,
                      int(round(1 / self.refresh_period)))
    self.assertEqual(results['jank_count'].value, 0)
    self.assertEqual(results['max_frame_delay'].value, 1)
    self.assertEqual(len(results['frame_lengths'].value), len(timestamps) - 1)
  ...
  def testSomeFramesTooShort(self):
    ...
    self.assertEqual(len(results['frame_lengths'].value), 9)
