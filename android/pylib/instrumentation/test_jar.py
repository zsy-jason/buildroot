...
class TestJar(object):
  ...
  def _GetTestsMissingAnnotation(self):
    ...
    for test_method in self.GetTestMethods().keys():
      annotations_ = frozenset(iter(self.GetTestAnnotations(test_method).keys()))
    ...
    if test_filter:
      ...
      tests = [
          sanitized_test_names[t] for t in unittest_util.FilterTestNames(
              list(sanitized_test_names.keys()), test_filter.replace('#', '.'))]
    else:
